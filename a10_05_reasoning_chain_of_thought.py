#　streamlit run a10_05_reasoning_chain_of_thought.py
prompting_at_first = """まず、クエリに答えるためにどのような文書が必要なのか、
一歩一歩注意深く考えてください。そして、各文書のタイトルとIDをプリントアウトする。
そして、IDをリストに整形する。
"""
    ...
    def _setup_sidebar(self):
        ...
        with st.sidebar:
            ...
            pattern_descriptions = {
                ...
                "Hypothesis-Test（仮説検証型）"     :
                    "仮説を立てて検証。バグ解析、科学実験、A/Bテストなどで使用。",
                "Tree-of-Thought（分岐探索型）"     :
                    "複数の思考経路を探索。パズル、最適化、ゲームAIなどに適用。",
                "Pros-Cons-Decision（賛否比較型）"  :
                    "メリット・デメリットを比較して決定。技術選定、意思決定に有効。",
                "Plan-Execute-Reflect（反復改良型）":
